    ...
    : audio(interruptSystem, inVchiqDevice)
{
  ...
  if (keyboard)
  {
    ...
    log(Log, "[CircleHostInterface] USB Keyboard device not detected");
  ...
  if (mouse)
  {
    ...
    log(Log, "[CircleHostInterface] USB Mouse device not detected");
}
...
void CircleHostInterface::resize(uint32_t desiredWidth, uint32_t desiredHeight)
{
  ...
  if ((screen->GetWidth() == desiredWidth) && (screen->GetHeight() == desiredHeight))
    return;
  if (mouse)
  {
    mouse->Release();
  ...
  case MouseEventMouseMove:
    ...
    if ((nPosX != instance->m_MouseX) || (nPosY != instance->m_MouseY))
    {
      ...
      */
      ...
      if (newEvent.mouseMotionX > MOUSE_DISPLACEMENT_MAX)
        newEvent.mouseMotionX = MOUSE_DISPLACEMENT_MAX;
      if (newEvent.mouseMotionX < MOUSE_DISPLACEMENT_MIN)
        newEvent.mouseMotionX = MOUSE_DISPLACEMENT_MIN;
      if (newEvent.mouseMotionY > MOUSE_DISPLACEMENT_MAX)
        newEvent.mouseMotionY = MOUSE_DISPLACEMENT_MAX;
      if (newEvent.mouseMotionY < MOUSE_DISPLACEMENT_MIN)
        newEvent.mouseMotionY = MOUSE_DISPLACEMENT_MIN;
