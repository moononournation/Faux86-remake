...
bool Config::parseCommandLine(int argc, char *argv[])
{
  ...
  if (audio.sampleRate < 22050)
    audio.sampleRate = AUDIO_DEFAULT_SAMPLE_RATE;
  else if (audio.sampleRate > 48000)
    audio.sampleRate = AUDIO_DEFAULT_SAMPLE_RATE;
  if (audio.latency < 32)
    audio.latency = 32;
  else if (audio.latency > 512)
    audio.latency = 512;

  if (cpuSpeed > 200)
    cpuSpeed = 200;
  if (cpuTiming < 1)
    cpuTiming = TIMING_INTERVAL;
  if (cpuTiming > 255)
    cpuTiming = TIMING_INTERVAL;
  if (cpuType > CPU_TYPE_386)
    cpuType = CPU_TYPE_V20;
  if (cpuType < CPU_TYPE_8086)
    cpuType = CPU_TYPE_V20;

  if (resw > 1024)
    resw = HOST_WINDOW_WIDTH;
  if (resh > 1024)
    resh = HOST_WINDOW_HEIGHT;
  if (renderQuality > 2)
    renderQuality = RENDER_QUALITY_LINEAR;
  if (monitorDisplay > 7)
    monitorDisplay = 0;
  // COM PORT AND IRQ
  // COM1 - 3F8h IRQ4
...
bool Config::setBootDrive(const char *str)
{
  ...
  if (strcmpi(str, "fd0") == 0)
    bootDrive = 0;
  else if (strcmpi(str, "fd1") == 0)
    bootDrive = 1;
  else if (strcmpi(str, "hd0") == 0)
    bootDrive = 128;
  else if (strcmpi(str, "hd1") == 0)
    bootDrive = 254;
  else if (strcmpi(str, "rom") == 0)
    bootDrive = 255;
  else
    bootDrive = atoi(str);
  return true;
