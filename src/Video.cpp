...
uint8_t Video::init(void)
{
...
#else
  for (y = 0; y < vm.config.framebuffer.width; y++)
  { // 400
    for (x = 0; x < vm.config.framebuffer.height; x++)
    { // 640
  ...
  */
  ...
  if (vga_RAM[0] == nullptr)
    log(Log, "[VIDEO] Error Allocating Video Memory for Bank 1");
  if (vga_RAM[1] == nullptr)
    log(Log, "[VIDEO] Error Allocating Video Memory for Bank 2");
  if (vga_RAM[2] == nullptr)
    log(Log, "[VIDEO] Error Allocating Video Memory for Bank 3");
  if (vga_RAM[3] == nullptr)
    log(Log, "[VIDEO] Error Allocating Video Memory for Bank 4");
  ...
  case VGA_MODE_TEXT:
    ...
    for (scy = start_y; scy <= end_y; scy++)
    {
      ...
      for (scx = start_x; scx <= end_x; scx++)
      {
        ...
        blink = attr >> 7;
        if (blinkenable)
          attr &= 0x7F; // enabling text mode blink attribute limits background color selection
...
void Video::vga_writememory(void *dummy, uint32_t addr, uint8_t value)
{
  uint8_t temp, plane;
  if ((vga_misc & 0x02) == 0)
    return; // RAM writes are disabled
  addr -= 0xA0000;
  addr = (addr - vga_membase) & vga_memmask; // TODO: Is this right?

  if (vga_gfxd[0x05] & 0x10)
  { // host odd/even mode (text)
    vga_RAM[addr & 1][addr >> 1] = value;
    return;
  }

  if (vga_seqd[0x04] & 0x08)
  { // chain-4
  ...
  case 0:
    for (plane = 0; plane < 4; plane++)
    {
      ...
      { // are we allowed to write to this plane?
        if (vga_gfxd[0x01] & (1 << plane))
        {                                                       // test enable set/reset bit for plane
...
uint8_t Video::vga_readmemory(void *dummy, uint32_t addr)
{
  ...
  else
  {
    ...
    for (plane = 0; plane < 4; plane++)
    {
      if (vga_gfxd[0x07] & (1 << plane))
      { // color don't care bit check
        if ((vga_RAM[plane][addr] & 0x0F) == (vga_gfxd[0x02] & 0x0F))
        {                    // compare RAM value with color compare register
