...
void PIT::tick()
{
  ...
  {
    if ((i == 2) && (mode[2] != 3))
      vm.pcSpeaker.setGateState(PC_SPEAKER_GATE_TIMER2, 0);
    if (active[i])
    {
      // timing_calls++;
      ...
      case 3: // square wave generator
        ...
        if (counter[i] <= 0)
        {
          ...
          if (i == 2)
            vm.pcSpeaker.setGateState(PC_SPEAKER_GATE_TIMER2, (reload[i] < 50) ? 0 : out[i]);
          counter[i] += reload[i];
        }
        break;
      default:
...
PIT::PIT(VM &inVM)
    : chandata{0, 0, 0}, accessmode{0, 0, 0}, bytetoggle{0, 0, 0}, effectivedata{0, 0, 0}, chanfreq{0, 0, 0}, active{0, 0, 0}, counter{0, 0, 0}, reload{0, 0, 0}, mode{0, 0, 0}, dataflipflop{0, 0, 0}, bcd{0, 0, 0}, rlmode{0, 0, 0}, latch{0, 0, 0}, out{0, 0, 0}, vm(inVM)
{
