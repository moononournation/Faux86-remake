...
void SoundBlaster::cmd(uint8_t value)
{
  ...
  if (waitforarg)
  {
...
#ifdef DEBUG_AUDIO
        log(Log, "[SOUNDBLASTER:cmd] Sound Blaster DSP block transfer size set to %u", blocksize);
#endif
        usingdma = 1;
        blockstep = 0;
        useautoinit = 0;
    ...
    case 0x40: // set time constant
      ...
      setsampleticks();
#ifdef DEBUG_AUDIO
      log(Log, "[SOUNDBLASTER:cmd] Sound Blaster time constant received, sample rate = %u", samplerate);
#endif
      break;
  ...
  default:
    ...
    {
#ifdef DEBUG_AUDIO
      log(Log, "[SOUNDBLASTER:cmd] Sound Blaster received unhandled command %02Xh", value);
#endif
    }
    break;
  }
...
{
#ifdef DEBUG_AUDIO
  log(Log, "[SOUNDBLASTER:portWriteHandler] port:%Xh value:%02X", portnum, value);
#endif
  portnum &= 0xF;
  switch (portnum)
  {
...
#ifdef DEBUG_AUDIO
      log(Log, "[SOUNDBLASTER:portWriteHandler] Sound Blaster received reset!");
#endif
    }
    lastresetval = value;
    break;
...
// if (lastread != portnum) log(Log,"[SoundBlaster::portReadHandler] port:%Xh", portnum);
#endif
  portnum &= 0xF;
  switch (portnum)
  {
...
#ifdef DEBUG_AUDIO
// if (lastread != portnum) log(Log,"[SoundBlaster::portReadHandler] value:%02X", ret);
// lastread = portnum;
#endif
  return true;
}
...
void SoundBlaster::tick()
{
  ...
  if (blockstep > blocksize)
  {
    vm.pic.doirq(sbirq);
#ifdef DEBUG_AUDIO
    log(Log, "[SoundBlaster::tick] Sound Blaster Send IRQ");
#endif
    if (useautoinit)
    {
      blockstep = 0;
