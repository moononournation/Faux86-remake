...
static uint32_t opl_index;
#if defined(OPLTYPE_IS_OPL3)
static uint8_t adlibreg[512]; // adlib register set (including second set)
static uint8_t wave_sel[44];  // waveform selection
#else
static uint8_t adlibreg[256]; // adlib register set
static uint8_t wave_sel[22];  // waveform selection
#endif

// Drum noise rand() replacement used to seed the generator.
static unsigned g_rndseed = 123456;
// Compute a pseudorandom integer in range [0, 32767]
static unsigned fast_rand(void)
{
  g_rndseed = (214013 * g_rndseed + 2531011);
  return (g_rndseed >> 16) & 0x7fff;
}
...
void change_feedback(Bitu chanbase, op_type *op_pt)
{
  ...
  if (feedback)
    op_pt->mfbi = (int32_t)(pow(FL2, (fltype)((feedback >> 1) + 8)));
  else
    op_pt->mfbi = 0;
}
...
static void clipit16(int32_t ival, int16_t *outval)
{
  if (ival < 32768)
  {
    if (ival > -32769)
...
void adlib_getsample(int16_t *sndptr, Bits numsamples)
{
  ...
  for (Bits cursmp = 0; cursmp < samples_to_process; cursmp += endsamples)
  {
    ...
    if (adlibreg[0x105] & 1)
      memset((void *)&outbufr, 0, (unsigned int)endsamples * sizeof(int32_t));
#endif
