...
{
  uint8_t ret;
  if (channels[channel].masked)
    return (128);
  if (channels[channel].autoinit && (channels[channel].count > channels[channel].reload))
    channels[channel].count = 0;
  if (channels[channel].count > channels[channel].reload)
    return (128);
...
bool DMA::portWriteHandler(uint16_t addr, uint8_t value)
{
  ...
  case 0x2: // channel 1 address register
    if (flipflop == 1)
      channels[1].addr = (channels[1].addr & 0x00FF) | ((uint32_t)value << 8);
    else
      channels[1].addr = (channels[1].addr & 0xFF00) | value;
#ifdef DEBUG_DMA
    if (flipflop == 1)
      log(Log, "[DMA] DMA channel 1 address register = %04X", channels[1].addr);
#endif
    flipflop = ~flipflop & 1;
    break;
  case 0x3: // channel 1 count register
    if (flipflop == 1)
      channels[1].reload = (channels[1].reload & 0x00FF) | ((uint32_t)value << 8);
    else
      channels[1].reload = (channels[1].reload & 0xFF00) | value;
    if (flipflop == 1)
    {
      if (channels[1].reload == 0)
        channels[1].reload = 65536;
      channels[1].count = 0;
#ifdef DEBUG_DMA
